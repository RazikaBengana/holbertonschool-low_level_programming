#ifndef LISTS_H
#define LISTS_H


#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>



/**
 * struct listint_s - singly linked list node structure
 *
 * @n: the integer value stored in the node
 * @next: a pointer to the next node in the list
 *
 * this structure defines a node for a singly linked list;
 * each node contains an integer value 'n' and a pointer 'next' that points
 * to the next node in the list;
 * this structure is used to create linked lists where each node
 * holds an integer value and a reference to the next node
 */

typedef struct listint_s
{
    int n;
    struct listint_s *next;
} listint_t;



/**
 * struct listadd_s - structure to hold addresses and link them
 * in a singly linked list
 *
 * @address: the address stored in the node
 * @next: a pointer to the next node in the list
 */

typedef struct listadd_s
{
    void *address;
    struct listadd_s *next;
} listadd_t;



/* _putchar function (if needed) */
int _putchar(char c);

size_t print_listint(const listint_t *h);
size_t listint_len(const listint_t *h);
listint_t *add_nodeint(listint_t **head, const int n);
listint_t *add_nodeint_end(listint_t **head, const int n);
void free_listint(listint_t *head);
void free_listint2(listint_t **head);
int pop_listint(listint_t **head);
listint_t *get_nodeint_at_index(listint_t *head, unsigned int index);
int sum_listint(listint_t *head);
listint_t *insert_nodeint_at_index(listint_t **head, unsigned int idx, int n);
int delete_nodeint_at_index(listint_t **head, unsigned int index);

/* advanced tasks */
listint_t *reverse_listint(listint_t **head);
listadd_t *add_node_to_list(listadd_t **list_head, const listint_t *node_address);
void free_list_nodes(listadd_t *list_head);
size_t print_listint_safe(const listint_t *list_head);
size_t free_listint_safe(listint_t **head);
listint_t *find_listint_loop(listint_t *head);


#endif /* LISTS_H */